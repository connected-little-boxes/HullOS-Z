...
void Frame::update()
{
	...
	if(colourBackSteps>0)
	{
		background.Red = background.Red + redBackStep;
		background.Blue = background.Blue + blueBackStep;
		background.Green = background.Green + greenBackStep;
		colourBackSteps--;
		if(colourBackSteps==0){
			background=targetBackColour;
		}
	}
...
void Frame::fadeBackToColour(Colour target, int noOfSteps)
{
	if (noOfSteps <= 0)
	{
		noOfSteps = 10;
	}
	redBackStep = (target.Red - background.Red) / noOfSteps;
	blueBackStep = (target.Blue - background.Blue) / noOfSteps;
	greenBackStep = (target.Green - background.Green) / noOfSteps;

	colourBackSteps = noOfSteps;
	targetBackColour = target;
}

void Frame::setBackColour(Colour target)
{
	background = target;
}
