...
{
    TRACELOGLN("Displaying MAX7219 message");

    if (strContains(options, "sticky"))
    {
        TRACELOGLN("  Setting a sticky message");
    ...
    else
    {
        ...
        {
            TRACELOGLN("    Currently displaying a sticky message");
            ...
            if(stickyTime>=max7219MessagesSettings.stickyMessageDurationMS)
            {
                ...
                TRACELOGLN("     Sticky message timed out");
                MAX7219displayingStickyMessage = false;
            }
            else
            {
                // sticky message takes priority - ignore this message
                TRACELOGLN("     Message ignored sticky message being displayed");
                return;
            }
        }
    }
    ...
    {
        TRACELOGLN("  Scrolling message");
        startMAX7219MessageScroll();
    }
    else
    {
        TRACELOGLN("  Not scrolling message");
        stopMAX7219MessageScroll();
    }

    if(strContains(options,"small"))
    {
        TRACELOGLN("  Small text");
        mp->setFont(_Fixed_5x3);
    }
    else
    {
        TRACELOGLN("  Large text");
        mp->setFont(_sysfont);
    }
    ...
    mp->update();
    TRACELOGLN("Done displaying MAX7219 message");
}
...
struct CommandItem MAX7219CommandOptionName = {
    "options",
    "max7219 message options (small,scroll,sticky, datestamp)",
...
int doSetMAX7219Message(char *destination, unsigned char *settingBase)
{
    TRACELOGLN("Doing MAX7219 display");
    ...
    char dateStampBuffer[MAX7219MAX_DATESTAMP_BUFFER_LENGTH];

    // make the buffer an empty string
    dateStampBuffer[0] = 0;

    if (strContains(options, "datestamp"))
    {        
        // this returns false if it doesn't work, but we ignore this because
        // we have already set the dateStampBuffer string to empty
        getDateAndTime(dateStampBuffer,MAX7219MAX_DATESTAMP_BUFFER_LENGTH );
    }

    snprintf(buffer, MAX_MESSAGE_LENGTH, "%s%s%s%s", dateStampBuffer, pre, message, post);

    displayMAX7219Message(buffer, options);

    TRACELOGLN("Done MAX7219 display");
    return WORKED_OK;
}
